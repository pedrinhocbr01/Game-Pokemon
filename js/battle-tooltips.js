...
text+="<p>"+(window.TT?TT('tip.basePower'):"Base power")+": "+value+"</p>";
}
...
text+="<p>"+(window.TT?TT('tip.accuracy'):"Accuracy")+": "+accuracy+"</p>";
...
// SS: selvagem — ícone da Dex se já registrado + Pokébola se já capturado (ao lado do level).
var dexBuf='';try{if(window.ssWildEv&&window.ssWildEv.id&&typeof toID==='function'&&toID(pokemon.speciesForme)===window.ssWildEv.id){if(window.ssWildEv.seen)dexBuf+=' <img src="/old/images/layout/th_pokedex.png" alt="Dex" title="Registrado na Pokédex" style="height:15px;vertical-align:middle;image-rendering:pixelated">';if(window.ssWildEv.caught)dexBuf+=' <img src="/sprites/itemicons/poke-ball.png" alt="" title="Já capturado" style="height:17px;vertical-align:middle">';}}catch(e){}
if(!illusionIndex||illusionIndex===1){
text+="<h2>"+name+genderBuf+(illusionIndex?'':levelBuf)+(illusionIndex?'':dexBuf)+"<br />";
...
text+="<p><small>"+(window.TT?TT('tip.hp'):"HP:")+"</small> (fainted)</p>";
}else if(pokemon.fainted){
text+="<p><small>"+(window.TT?TT('tip.hp'):"HP:")+"</small> <span class=\"gray\">0/"+pokemon.maxhp+" (fainted)</span></p>";
}else if(this.battle.hardcoreMode){
if(serverPokemon){
var status=pokemon.status?" <span class=\"status "+pokemon.status+"\">"+pokemon.status.toUpperCase()+"</span>":'';
text+="<p><small>"+(window.TT?TT('tip.hp'):"HP:")+"</small> "+serverPokemon.hp+"/"+serverPokemon.maxhp+status+"</p>";
}
}else{
var exacthp='';
if(serverPokemon){
...
text+="<p><small>"+(window.TT?TT('tip.hp'):"HP:")+"</small> "+Pokemon.getHPText(pokemon,this.battle.reportExactHP)+exacthp+_status;
if(clientPokemon){
...
// Story Showdown: Nature + EVs que o SELVAGEM dá ao ser derrotado (só em batalha selvagem; vem do /safari/encounter).
try{
if(window.ssWildEv&&typeof toID==='function'&&toID(pokemon.speciesForme)===window.ssWildEv.id){
if(window.ssWildEv.nature)text+="<p><small>"+(window.TT?TT('tip.nature'):'Nature:')+"</small> "+BattleLog.escapeHTML(window.ssWildEv.nature)+"</p>";
var _ssEv=window.ssWildEv.evs;
if(_ssEv){var _ssLab={hp:'HP',atk:'Atk',def:'Def',spa:'SpA',spd:'SpD',spe:'Spe'};var _ssParts=[];
['hp','atk','def','spa','spd','spe'].forEach(function(k){if(_ssEv[k]>0)_ssParts.push('+'+_ssEv[k]+' '+_ssLab[k]);});
if(_ssParts.length)text+="<p><small>"+(window.TT?TT('tip.evYield'):'EVs on defeat:')+"</small> "+_ssParts.join(', ')+"</p>";}
}
}catch(_ssE){}
...
return"<p><small>Spe</small> "+max+" <small>"+(window.TT?TT('tip.beforeStatStage'):"(before stat stage changes)")+"</small></p>";
}
return"<p><small>Spe</small> "+min+" to "+max+" <small>"+(window.TT?TT('tip.beforeStatStage'):"(before stat stage changes)")+"</small></p>";
}
if(this.battle.tier.includes('Random')){
return"<p><small>Spe</small> "+min+" or "+ev84+" <small>"+(window.TT?TT('tip.beforeExternal'):"(before external modifiers)")+"</small></p>";
}else if(this.battle.tier.includes("Let's Go")){
return"<p><small>Spe</small> "+min+"<small class=\"gray\">&ndash;"+ev0+"&ndash;</small>"+max+" <small>"+(window.TT?TT('tip.beforeExternal'):"(before external modifiers)")+"</small></p>";
}else{
return"<p><small>Spe</small> "+min+"<small class=\"gray\">&ndash;"+ev0+"&ndash;"+ev252+"&ndash;</small>"+max+"<br><small>"+(window.TT?TT('tip.beforeExternal'):"(before external modifiers)")+"</small></p>";
};
}
...
text='<small>'+(window.TT?TT('tip.possibleAbilities'):'Possible abilities:')+'</small> '+abilityData.possibilities.join(', ');
}
return text;
